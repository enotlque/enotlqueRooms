...
# --------------------------------------------------------------------------
# КООРДИНАТЫ ДЛЯ РАЗМЕРА 1200x640
# --------------------------------------------------------------------------

DEBUG_GRID = True
DEBUG_LOG = True  # Включаем отладку в консоль

# Аватар
AVATAR_CENTER = (600, 140)
AVATAR_RADIUS = 72
...
USERNAME_Y = 260
USERNAME_MAX_WIDTH = 380
USERNAME_FONT_SIZE = 34

# "На сервере с ..." 
JOINED_CENTER_X = 600
JOINED_Y = 510
JOINED_MAX_WIDTH = 380
JOINED_FONT_SIZE = 16
...
# ЦВЕТА СТАТУСОВ
COLOR_ONLINE = (67, 181, 129, 255)
COLOR_IDLE = (250, 166, 26, 255)
COLOR_DND = (237, 66, 69, 255)
COLOR_OFFLINE = (128, 128, 128, 255)


def _get_status_color(member: discord.Member) -> tuple:
    """Возвращает цвет рамки в зависимости от статуса пользователя."""
...
async def get_member_room_options(cursor, member: discord.Member) -> list:
    """Все комнаты, где у пользователя есть роль."""
    # Получаем ID ролей пользователя в виде строк
    member_role_ids = {str(r.id) for r in member.roles}
    
    if DEBUG_LOG:
        print(f"[DEBUG] Роли пользователя {member.name}: {member_role_ids}")
    
    result = await cursor.execute('SELECT room_name, role_id, creation_date FROM room_leadership')
    rows = cursor.fetchall()
    
    rooms = []
    for room_name, role_id, creation_date in rows:
        # Приводим role_id к строке для безопасного сравнения
        if str(role_id) in member_role_ids:
            rooms.append((room_name, creation_date))
            if DEBUG_LOG:
                print(f"[DEBUG] Найдена комната: {room_name} (role_id={role_id})")
    
    if DEBUG_LOG:
        print(f"[DEBUG] Всего найдено комнат для {member.name}: {len(rooms)}")
    
    return rooms


async def _get_room_name(cursor, member: discord.Member):
    """Получает комнату для отображения в профиле."""
    rooms = await get_member_room_options(cursor, member)
    
    if DEBUG_LOG:
        print(f"[DEBUG] _get_room_name для {member.name}: найдено {len(rooms)} комнат")
    ...
    # Если только одна комната - берем ее
    if len(rooms) == 1:
        if DEBUG_LOG:
            print(f"[DEBUG] Только одна комната: {room_names[0]}")
        return room_names[0]
    
    # Если комнат несколько - берем выбранную пользователем
    ...
    if chosen_name and chosen_name in room_names:
        if DEBUG_LOG:
            print(f"[DEBUG] Используем выбранную комнату: {chosen_name}")
        return chosen_name
    
    # Если ничего не выбрано - берем первую комнату
    if DEBUG_LOG:
        print(f"[DEBUG] Берем первую комнату: {room_names[0]}")
    return room_names[0]
...
async def create_profile_image(cursor, member: discord.Member, guild: discord.Guild) -> io.BytesIO:
    ...
    if DEBUG_LOG:
        print(f"[DEBUG] Итоговая комната для {member.name}: '{room_name}'")
    ...
    # Получаем цвет статуса пользователя
