...
_release_connection = None  # async функция для возврата соединения в пул (обязательна для реального пулинга)
...
def set_connection_factory(factory, release_factory=None):
    """factory - async функция без аргументов, возвращающая asyncpg.Connection
    (в main.py это уже существующая get_db_connection).
    release_factory - async функция(conn), возвращающая соединение в пул
    (в main.py это release_db_connection). Без неё пришлось бы закрывать
    соединение через conn.close(), что для соединения из pool.acquire()
    реально рвёт TCP-сессию вместо возврата в пул."""
...
def setup_activity_tracking(bot: commands.Bot, cursor, db_connection_factory, db_release_factory=None):
    """Вызывается один раз из main.py."""
    set_cursor(cursor)
    set_connection_factory(db_connection_factory, db_release_factory)
